...
def contamination(results,image,aperture,target_coord_pixel,target_tmag,nb_coords_pixel,nb_tmags,wcs,median_bkg_flux,prepend_err_msg=''):
    ...
    # Store to results
    results.fit = SimpleNamespace()
    results.fit.fitted_image = fitted_image
    results.fit.intercept = Plane.intercept.value
    results.fit.Plane = Plane
    results.fit.TargetStar = TargetStar
    results.fit.Neighbours = Neighbours
    results.fit.slope_y = Plane.slope_y.value
    results.fit.slope_x = Plane.slope_x.value
    results.fit.neighbour_flux_ap = neighbour_flux
    results.fit.target_flux_ap = target_flux
    results.fit.bkg_flux_ap = bkg_flux
    results.fit.fraction_contamination_ap = fraction_ap_contamination
    results.fit.fraction_bkg_change = fraction_bkg_change
...
def refine_aperture(results,tic,ra,dec,wcs,aperture,threshold,image,prepend_err_msg=''):
    ...
    # Store to results (get plain numbers from the AstroPy instance)
    results.neighbours_all = SimpleNamespace() 
    results.neighbours_all.mag = nb_tmags
    results.neighbours_all.ra = np.array([coord.ra.deg  for coord in nb_coords])
    results.neighbours_all.dec = np.array([coord.dec.deg for coord in nb_coords])
    ...
    # Store to results (get plain numbers from the AstroPy instance)
    results.target = SimpleNamespace()
    results.target.mag = target_tmag
    results.target.ra = target_coord.ra.deg
    results.target.dec = target_coord.dec.deg
    results.target.pix = target_coord_pixel
    ...
    # Store to results (get plain numbers from the AstroPy instance)
    results.neighbours_used = SimpleNamespace()
    results.neighbours_used.mag = nb_tmags
    results.neighbours_used.ra = np.array([coord.ra.deg  for coord in nb_coords])
    results.neighbours_used.dec = np.array([coord.dec.deg for coord in nb_coords])
    results.neighbours_used.pix = nb_coords_pixel
...
def extract_lc(fitsFile,outputdir=None,return_msg=True):

    # Output name
    outputdir = Path('processed') if outputdir is None else Path(outputdir)
    outputname = Path(fitsFile.stem+'_corrected.pickled')
    ...
    results.ra = None
    results.dec = None
    results.headers = None # Headers from the original FITS file
    results.fit = None
    results.neighbours_all = None
    results.neighbours_used = None
    results.target = None
    results.aperture_threshold = None # HDU to store tabular information
    ...
    while True:
        try:
            HDUL.append( fits.getheader(fitsFile.as_posix(), ext=ext).tostring() )
            ext += 1
        # If all headers have been read, break
        except IndexError:
            break
        except Exception as e:
            e_name = e.__class__.__name__
            print(f'Unexpected exception when reading headers from FITS Excepion -> {e_name}: {e}.')
            break
    results['headers'] = HDUL

    # Load the TESS taret pixel file
    try:
        tpf = lk.TessTargetPixelFile(fitsFile.as_posix())
    except Exception as e:
        # Save results
        err_msg = f'"lightkurve.TessTargetPixelFile()" could not open file {fitsFile.as_posix()}. Exception -> {e_name}: {e}'
        results.tag = err_msg
    ...
    results.masks.aperture = ap_mask
    results.masks.background = ap_bkg
    ...
    #                         'time':tpf.time}
    # Store to results
    results.centroids = SimpleNamespace()
    results.centroids.col = centroid_col
    results.centroids.row = centroid_row
    results.centroids.sqrt_col2_row2 = sqrt_col2_row2
    results.centroids.time = tpf.time
    ...
    #                      'time':lc_raw.time}
    # Store to results
    results.lc_raw = SimpleNamespace()
    results.lc_raw.flux = lc_raw.flux
    results.lc_raw.time = lc_raw.time
    ...
    # results['lc_raw_nonan'] = {'flux':lc.flux,\
    #                            'time':lc.time}
    results.lc_raw_nonan = SimpleNamespace()
    results.lc_raw_nonan.flux = lc.flux
    results.lc_raw_nonan.time = lc.time
    ...
    if npc == 0:
        # Save results
        results['tag'] = id_msg+'None PC used, no detrended done.'
        picklefile = open(outputpath.as_posix(), 'wb')
        pickle.dump(results, picklefile)
        picklefile.close()
        if return_msg: return err_msg
        return
